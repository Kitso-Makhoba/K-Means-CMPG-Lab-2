...
plt.show()
avg_energy = df.groupby("Cluster")["Energy"].mean()
print("Average Energy per Cluster:\n", avg_energy)
print(f"Cluster with lowest energy: {avg_energy.idxmin()}")
min_energy_node = df.loc[df["Energy"].idxmin()]
print(f"Critical Node: {min_energy_node['Node']} in Cluster {min_energy_node['Cluster']}")